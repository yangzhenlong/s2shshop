...
<html>
  <head>
	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({
			    url:'category_queryJoinAccount.action',//获取datagrid_data.json文件中的内容
			    ...
			    columns:[[
			        ...
			        {field:'hot',title:'是否热点',width:100,align:'right',
			        	//使用styler属性，设置该列满足条件的值的样式
			        	formatter: function(value,row,index){
			        		console.info("value的类型：" + typeof(value));
							if (value){
								return '<input type="checkbox" value="true" checked="checked" disabled="disabled"/>';
							}else{
								return '<input type="checkbox" value="false"  disabled="disabled"/>';;
							}
						}
			        },
			        {field:'account',title:'所属管理员',width:100,align:'right',
			        	formatter:function(value,row,index){
			...
			$('#ss').searchbox({ 
				searcher:function(value,name){ 
					...
					$('#dg').datagrid('load',{
						type: value //通过input中的name值 重新搜索
					});
