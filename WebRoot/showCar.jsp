	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$(".text").change(function(){
				...
				//1.验证填写数据是否正确
				if(!isNaN(number) && parseInt(number)==number && number>0){ //isNan：非数字；parseInt：自然数取正；
					//alert("合法");
					$(this).attr("lang",number);//将新值设置到lang属性中
					//2.发送ajax请求，请求参数为数量和商品Id，返回参数为总价格
					//获取商品Id
					var productId = $(this).parents("tr:first").attr("lang");
					//alert(productId);
					//$.post("url",{json格式参数},function(返回参数){回调函数})
					$.post("sorder_update.action",{number:number,'product.id':productId},function(total){
						//alert("总价格："+total);
						//更新总计的值
						$("#total").html(total);
						$("#zhifu").html((parseInt(total)+20.00)+".00");
					});
					//更新小计
					var price = $(this).parents("td:first").prev().find("span").html();//单个商品单价 prev:前一个元素
					//alert(price);
					$(this).parents("td:first").next().find("span").html(price*number);
					//更新需付款
				}else{
					//alert("不合法");
					this.value = $(this).attr("lang");	//lang：该元素的语言属性值
				}
			});
		});
		...
		<div class="header">
			<div class="header_container">
				...
				<div class="top_bar clear">
					<!--头部小导航-->
					<div class="welcom fl">${sessionScope.user.name },欢迎光临LEISUPET SHOP!</div>
			...
			<div id="shopping_cart">
				...
				<table class="data-table cart-table" cellpadding="0" cellspacing="0">
					...
					<tr lang="${sorder.product.id}">
						...
						<td class="align_center vline">￥<span class="price">${sorder.number * sorder.price }</span>
						</td>
				...
				<div class="totals">
					<table id="totals-table">
						<tbody>
							<tr>
								...
								<td class="align_right" style=""><strong>
								￥<span id="total" class="price">${sessionScope.forder.total }</span>
								</strong>
								</td>
							</tr>
							<tr>
								<td width="60%" colspan="1" class="align_left">运费</td>
								<td class="align_right" style=""><span class="price">￥20.00</span>
								</td>
							</tr>
							<tr>
								<td width="60%" colspan="1" class="align_left total"><strong>需付款</strong>
								</td>
								<td class="align_right" style=""><span class="total">￥<strong id="zhifu">${sessionScope.forder.total+20 }</strong>
								</span>
								</td>
							</tr>
						</tbody>
					...
					<div class="action_buttonbar">
						<button type="button" title="" class="checkout fr"
							...
							<font><a href="${shop }/user/confirm.jsp">结帐</a></font>
						</button>
